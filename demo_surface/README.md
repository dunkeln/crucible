...
At this stage, the demo surface reads from a local HTTP API defined in [api.py](/D:/crucible/demo_surface/api.py). The API serves a sample run from [data/demo_run.json](/D:/crucible/demo_surface/data/demo_run.json), and the view layer in [main.py](/D:/crucible/demo_surface/main.py) fetches that payload before rendering it. The same API also serves a browser landing page at `/` and a human-readable run view at `/view`. That means the demo is now connected through an API boundary even though there is still no model inference yet, no dataset loader, and no verifier service behind the scene. The program simply renders one sample run with the fields that matter to Crucible: the task, the student attempt, the verifier result, the reward, the teacher repair, and a tiny curated row export. This is intentional. The repository guidance says to keep the slice bounded and to reward evidence instead of presentation alone, so the first connected version is only meant to show the loop clearly.
